...
/*   Updated: 2026/01/11 21:29:04 by mtarrih          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static mlx_t	*open_scaled_window(const char *title)
{
	...
	mlx_set_window_size(mlx, (int32_t)(width * MONITOR_SCALE), (int32_t)(height
			* MONITOR_SCALE));
	mlx_set_window_pos(mlx, width / 2 - mlx->width / 2, height / 2 - mlx->height
		/ 2);
	return (mlx);
}
...
int	main(int ac, char *av[])
{
	...
	if (parse.ok)
	{
		...
		if (init_textures(parse) && init_map(parse))
		{
			...
			if (load_player((t_vector2){parse.player_pos.x + 0.5,
